...
- **Palette (light):** cream `#FAFAF2` page, butter `#E8EBB2`, lilac `#DCD3E8`, warm peach and soft sky colour blocks, one deep green `#164E3E` for text, hairlines, buttons and the dark card; `#3F5A50` for secondary text. Mapped to semantic tokens in `src/routes/layout.css`: `background`/`foreground` = cream/green, `primary` = green on butter, `secondary` = butter, `accent` = lilac, `warm` = peach, `cool` = sky, `muted-foreground` = the soft green, `border` = green hairlines. Dark mode inverts the roles with deeper, muted counterparts for every colour surface. Always use the semantic tokens; never hardcode a hex in a component.
